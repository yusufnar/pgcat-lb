...
# Stops both pg-replica1 and pg-replica2 to test PgCat behavior
...
# Initial status
echo "[$(date +%H:%M:%S)] Initial PgCat pool status:"
...
# Wait for PgCat to detect
echo ""
echo "[$(date +%H:%M:%S)] Waiting 5s for PgCat to detect..."
sleep 5

# Show status after stop
echo ""
echo "[$(date +%H:%M:%S)] PgCat pool status (both replicas stopped):"
PGPASSWORD=admin psql -h 127.0.0.1 -p 6432 -U admin -d pgcat -c "SHOW POOLS" 2>/dev/null
...
echo "  Note: Queries may fail if PgCat can't route to any replica."
...
# Final status
echo ""
echo "[$(date +%H:%M:%S)] Final PgCat pool status:"
PGPASSWORD=admin psql -h 127.0.0.1 -p 6432 -U admin -d pgcat -c "SHOW POOLS" 2>/dev/null
