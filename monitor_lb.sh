...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          PGCAT LOAD BALANCING MONITOR                       ║"
...
while true; do
    ...
    for i in {1..10}; do
        ip=$(PGPASSWORD=secret psql -h $PGCAT_HOST -p $PGCAT_PORT -U $USER -d $DB -t -c "SELECT inet_server_addr() as node, pg_sleep(0.2);" 2>/dev/null | awk -F'|' '{print $1}' | tr -d '[:space:]')
