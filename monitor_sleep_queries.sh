...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          PGCAT LONG QUERY (SLEEP) MONITOR                   ║"
...
while true; do
    ...
    for i in {1..4}; do
        (
            PGPASSWORD=secret psql -h $PGCAT_HOST -p $PGCAT_PORT -U $USER -d $DB -c "SELECT inet_server_addr() as node, pg_sleep(20);" 2>&1 | while read line; do
