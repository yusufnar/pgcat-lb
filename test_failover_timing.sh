...
# Function to check if PgCat can reach pg-replica1
...
echo "--- PgCat Pool Status ---"
PGPASSWORD=admin psql -h 127.0.0.1 -p 6432 -U admin -d pgcat -c "SHOW POOLS" 2>/dev/null
...
# Poll for PgCat to detect failure (banned)
echo "[$(date +%H:%M:%S)] Waiting for PgCat to detect failure..."
while true; do
    STATUS=$(get_replica1_reachable)
    if [ "$STATUS" == "not_reachable" ]; then
        DOWN_TIME=$(date +%s)
        DETECT_DURATION=$((DOWN_TIME - STOP_TIME))
        echo "[$(date +%H:%M:%S)] ✅ PgCat no longer routing to pg-replica1"
...
# Poll for recovery
echo "[$(date +%H:%M:%S)] Waiting for PgCat to recover pg-replica1..."
while true; do
    STATUS=$(get_replica1_reachable)
    if [ "$STATUS" == "reachable" ]; then
        UP_TIME=$(date +%s)
        RECOVERY_DURATION=$((UP_TIME - START_TIME))
        echo "[$(date +%H:%M:%S)] ✅ PgCat routing to pg-replica1 again"
